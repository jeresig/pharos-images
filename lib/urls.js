...
const genURL = (locale, base, path) => {
    // Use sub-domains to track the locale
    base = (locale === defaultLocale || !locale || env === "development" ?
        base :
        base.replace("://", `://${locale}.`));

    // Only use a query string in dev mode and if we're
    // on a non-default locale
    if (env === "development" && locale && locale !== defaultLocale) {
        const prefix = /\?/.test(path) ? "&" : "?";
        path += `${prefix}lang=${locale}`;
    }
...
const baseData = () => {
    return process.env.BASE_DATA_URL ||
        `http://localhost:${process.env.PORT}/data`;
};
